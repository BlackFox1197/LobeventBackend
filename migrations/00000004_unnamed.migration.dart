...
class Migration4 extends Migration { 
  ...
  Future upgrade() async {
   		database.createTable(SchemaTable("_User_EventStatus", [SchemaColumn("id", ManagedPropertyType.bigInteger, isPrimaryKey: true, autoincrement: true, isIndexed: false, isNullable: false, isUnique: false),SchemaColumn("status", ManagedPropertyType.integer, isPrimaryKey: false, autoincrement: false, isIndexed: true, isNullable: false, isUnique: false)]));
		...
		database.addColumn("_User_EventStatus", SchemaColumn.relationship("userId", ManagedPropertyType.bigInteger, relatedTableName: "_User", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
		database.addColumn("_User_EventStatus", SchemaColumn.relationship("eventId", ManagedPropertyType.bigInteger, relatedTableName: "_Event", relatedColumnName: "id", rule: DeleteRule.nullify, isNullable: true, isUnique: false));
  ...
  @override
  Future seed() async {
		final users = [["Fox", "allls", "alls"], ["Fox2", "allls", "alls"]];

		for (final user in users) {
			await database.store.execute("INSERT INTO _User (username, hashedPassword, salt) Values (@username, @hashedPassword, @salt)", substitutionValues: {
				"username" : user[0],
